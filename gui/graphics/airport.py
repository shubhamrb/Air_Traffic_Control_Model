...
class LocaliserItem(QGraphicsItem):
	...
	def updateFromSettings(self):
		...
		if self.runway.LOC_range == None:
			...
			self.draw_GS_up_to = 0 if self.runway.GS_range == None else min(self.runway.LOC_range, self.runway.GS_range)
...
class TarmacSectionItem(QGraphicsItem):
	...
	'''
	def __init__(self, name, surface, path, parent=None):
...
class AirportGroundNetItem(QGraphicsItem):
	def __init__(self, gndnet):
		...
		self.apron_item = TaxiRouteItem(self, apron_edges, 'ground_route_apron')
		self.twy_items = []
		for twy_name in gndnet.taxiways():
			...
			twy_item = TaxiRouteItem(self, twy_qedges, 'ground_route_taxiway')
...
class AirportLinearObjectItem(QGraphicsItem):
	...
	def paint(self, painter, option, widget):
		if self.path != None:
			painter.setPen(new_pen(settings.colour(self.colour_name), width=self.width))
			painter.drawPath(self.path)
