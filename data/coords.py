...
def read_coord(s):
	...
	if match: # Deg-min-sec format
		...
		if match.group(3) != None: # minutes
			decdeg += float(match.group(3)) / 60
			if match.group(5) != None: # seconds
...
class EarthCoords:
	...
	def fromString(s):
		...
		if len(tokens) != 2:
			raise ValueError('String should contain one and only comma')
		else:
...
def WGS84_geodetic_to_cartesian_metres(geodetic, ftAMSL):
	'''
...
def cartesian_metres_to_WGS84_geodetic(x, y, z):
	...
	ftAMSL = (k + e2 - 1) * sqrtDDpZZ/k
	return EarthCoords(degrees(lat), degrees(lon)), m2ft * ftAMSL
